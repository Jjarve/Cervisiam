...
public class SalesDomainControllerImpl implements SalesDomainController {
	...
	@Override
	public void registerSale(Sale saleToRegister) {

        // Begin transaction
        Transaction tx = session.beginTransaction();

        for (SoldItem item : saleToRegister.getSoldItems()) {
            // Associate with current sale
            item.setSale(saleToRegister);
        ...
        session.save(saleToRegister);

        // end transaction
        tx.commit();
        model.getPurchaseHistoryTableModel().addRow(saleToRegister);
